...
/**
 * 簡易快取：避免 LINE 每傳一次就重抓 TWSE 全部資料
 */
let twseCache = {
  sectorReport: null,
  updatedAt: 0
...
/**
 * 備援產業分類表：
 * 如果 TWSE 上市公司基本資料 API 抓不到，至少常見權值與熱門股還能分類。
 * 後續可以慢慢擴充。
 */
...
async function createTWSESectorRankingReport() {
  try {
    ...
    if (!dailyRows || dailyRows.length === 0) {
      return (
        "【YORU LAB 上市資金排行】\n\n" +
        "目前沒有抓到 TWSE 上市每日成交資料，可能是資料尚未更新或 TWSE API 暫時無回應。\n\n" +
        "請稍後再試。"
      );
    }
    ...
    if (institutionalList.length === 0) {
      reply += "目前沒有抓到三大法人買超資料，可能是今日資料尚未更新。\n";
    } else {
    ...
    return (
      "【YORU LAB 上市資金排行】\n\n" +
      "TWSE 官方資料抓取或計算時發生錯誤。\n\n" +
      "請到 Render Logs 查看錯誤。如果是 404 或欄位錯誤，代表 TWSE API 欄位或路徑可能有調整。"
    );
  }
}
 ...
 * 如果這個路徑失敗，會自動回到 FALLBACK_SECTOR_MAP。
 */
async function fetchTWSECompanySectorMap() {
  ...
  try {
    ...
    console.error("fetchTWSECompanySectorMap error, fallback will be used:", error);
  }
...
async function fetchJSON(url) {
  const response = await fetch(url, {
    headers: {
      "User-Agent": "Mozilla/5.0 YORU-LAB-LINE-BOT"
    }
  });
